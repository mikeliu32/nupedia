	...
	tinymce.create('tinymce.plugins.MediaPlugin', {
		init : function(ed, url) {
			...
			ed.onPreInit.add(function() {
				...
				// ooki (view to edit)
				ed.parser.addNodeFilter('span', function(nodes) {
					var i = nodes.length;

					while (i--) {
						//self.objectToImg(nodes[i]);
						if (/ooki(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)/.test(nodes[i].attr('class'))) {
							var node = nodes[i];
							cla = (node.attr('class') == 'ookiFlash' ? 'mceItemFlash' : 'mceItemWindowsMedia');
							arg = JSON.parse('{'+node.attr('title')+'}');
							
							type = (cla == "mceItemWindowsMedia" ? "windowsmedia" : "flash");
							src = node.firstChild.attr('href');
							w = (arg['width'] || 320);
							h = (arg['height'] || 250);
		...
		imgToObject : function(node, args) {
			...
			// ooki (edit to view)
			if (/mceItemFlash|mceItemWindowsMedia/.test(node.attr('class'))) {
				//write(Flash|ShockWave|WindowsMedia|QuickTime|RealMedia)
				cla = (node.attr('class').indexOf('mceItemFlash') > -1 ? 'Flash' : 'WindowsMedia');
				replacement = new Node('span', 1);
				replacement.attr({
					'class':'ooki'+cla,
					title:"'width':'" + data.width + "','height':'" +data.height + "'"
				});
				a = new Node('a', 1);
				a.attr({
					'class':'ooki_player',
					href:(data.params.src || '')
				});
				value = new Node('#html', 3);
				value.value = 'ooki Media player';
				a.append(value);
